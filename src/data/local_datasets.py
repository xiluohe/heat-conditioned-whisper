...
class TS_ASR_DatasetSuperclass:
    ...
    def __init__(self, cutsets, text_norm=lambda x: x, do_augment=False, use_timestamps=False,
                 empty_transcript_ratio=0.00, musan_noises=None, audio_path_prefix=None,
                 audio_path_prefix_replacement=None,
                 max_timestamp_pause=0.0, vad_from_alignments=False,
                 dataset_weights=None,
                 num_heat_channels=2, heat_assignment_method='first-available',
                 *args,
                 **kwargs):

        self.num_heat_channels = num_heat_channels
        self.heat_assignment_method = heat_assignment_method
...
class TS_ASR_HEAT_Dataset(TS_ASR_DatasetSuperclass, Dataset):
    ...
    def cut_to_sample(self, cut, stream_id): 
        audio = cut.load_audio()
        ...
        if self.heat_assignment_method == 'alternating':
            ...
            for sup in all_supervisions:
                ...
                assignment_channel = possible_channels[0] if len(possible_channels) > 0 else last_seg_end.index(min(last_seg_end))
        ...
        elif self.heat_assignment_method == 'recency-continuity':
            last_seg_end = [0 for _ in range(self.num_heat_channels)]
            for sup in all_supervisions:
                start, end = sup.start, sup.end_
                ...
                heat_assignments[assignment_channel].append(sup)
                last_seg_end[assignment_channel] = max(end, last_seg_end[assignment_channel])
        elif self.heat_assignment_method == 'speaker-continuity':
            last_seg_end = [0 for _ in range(self.num_heat_channels)]
            ...
            for sup in all_supervisions:
                ...
                if len(possible_channels) == 2:
                    if speaker_id in last_seg_spkr and last_seg_spkr.index(speaker_id) in possible_channels: # Pick based on speaker
                        possible_channels = [last_seg_spkr.index(speaker_id)] 
                    elif last_seg_end.index(max(last_seg_end)) in possible_channels: # Pick based on recency-continuity
                        possible_channels = [last_seg_end.index(max(last_seg_end))]
...
class LhotseLongFormHeatDataset(Dataset):
    def __init__(self, cutset: CutSet, is_multichannel: bool = False, use_timestamps: bool = False,
                 ...
                 audio_path_prefix=None, audio_path_prefix_replacement=None, references: CutSet = None,
                 soft_vad_temp=None, 
                 num_heat_channels=2, heat_assignment_method='first-available', diar_type='oracle',
                 **kwargs):
        self.cutset = cutset
        self._references = references
    ...
    def prepare_vad_mask(self, cut: Cut, stream_id: int):
        ...
        for sid in self.get_cut_spks(cut): # all speakers in cut 
            ...
            merged_supervisions = self.merge_supervisions(target_spk_cut)
            all_supervisions = all_supervisions + merged_supervisions
        vad_mask, heat_mask = self._prepare_heat_vad_mask(cut, stream_id, all_supervisions, self.heat_assignment_method)
    ...
    @staticmethod
    def _prepare_heat_vad_mask(cut, stream_id, all_supervisions, heat_assignment_method='first-available'):
        num_channels = 2

        heat_assignments = [[] for _ in range(num_channels)]
        
        if heat_assignment_method == 'alternating':
            ...
            for sup in all_supervisions:
                ...
                last_seg_end[assignment_channel] = max(end, last_seg_end[assignment_channel])
                last_channel_assigned = assignment_channel
        elif heat_assignment_method == 'recency-continuity':
            last_seg_end = [0 for _ in range(num_channels)]
            for sup in all_supervisions:
                start, end = sup.start, sup.end_
                ...
                heat_assignments[assignment_channel].append(sup)
                last_seg_end[assignment_channel] = max(end, last_seg_end[assignment_channel])
        elif heat_assignment_method == 'speaker-continuity':
            last_seg_end = [0 for _ in range(num_channels)]
...
def build_dataset(cutset_paths: List[Union[str, Path]], data_args: DataArguments, dec_args: DecodingArguments, text_norm, container, diar_cutset_paths=None):
    ...
    if data_args.diar_type != 'oracle' and diar_cutset_paths is not None:
        if len(diar_cutset_paths) == 0:
            raise ValueError("'diar_cutset_paths' is not None and empty. Please provide valid 'diar_cutset_paths' for the dataset")
    ...
    if data_args.use_heat_diar:
        return LhotseLongFormHeatDataset(cutset=cutset, references=refs,
                                    ...
                                    num_heat_channels=data_args.num_heat_channels, heat_assignment_method=data_args.heat_assignment_method,
